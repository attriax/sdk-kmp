...
/**
 * Dependency-free SHA-256 (FIPS 180-4) producing the 32-byte digest used on the
 * Apple App Attest path as the `clientDataHash` (`SHA256(nonce)` the server
 * recomputes to verify the attestation binds the exact nonce it issued).
 *
 * Kept pure and in `commonMain` (no CommonCrypto / platform cinterop) so the exact
 * digest is covered by a known-answer test — see `AttriaxSha256Test`. The Apple
 * provider ([com.attriax.sdk.apple], iosMain) calls [attriaxSha256] and wraps the
 * result as `NSData`.
 */
...
/** SHA-256 of [message], returning the 32-byte big-endian digest. */
internal fun attriaxSha256(message: ByteArray): ByteArray {
